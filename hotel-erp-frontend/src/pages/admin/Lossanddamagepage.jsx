...
const parseImages = (jsonStr) => {
    ...
    try {
        ...
        return jsonStr && jsonStr.startsWith("http") ? [{ url: jsonStr }] : [];
    }
};

const STATUS_CFG = {
    Pending: { label: "Chờ xử lý", bg: "#fff7ed", color: "#c2410c", dot: "#f97316" },
    Confirmed: { label: "Đã xác nhận", bg: "#f0fdf4", color: "#15803d", dot: "#22c55e" },
    Waived: { label: "Miễn trừ", bg: "#f8fafc", color: "#475569", dot: "#94a3b8" },
};

// ─── Toast ────────────────────────────────────────────────────────────────────
function Toast({ id, msg, type = "success", onDismiss }) {
    const cfg = {
        success: { bg: "#ffffff", border: "#e2e8f0", text: "#1a202c", icon: "check_circle", iconColor: "#22c55e" },
        error: { bg: "#ffffff", border: "#e2e8f0", text: "#1a202c", icon: "error", iconColor: "#ef4444" },
    }[type] || { bg: "#ffffff", border: "#e2e8f0", text: "#1a202c", icon: "info", iconColor: "#3b82f6" };
    ...
    return (
        <div style={{
            ...
            pointerEvents: "auto", marginBottom: 12, padding: "14px 20px",
            animation: "toastIn .4s cubic-bezier(.22,1,.36,1) forwards", minWidth: 300,
            display: "flex", alignItems: "center", gap: 12
        }}>
            <span className="material-symbols-outlined" style={{ color: cfg.iconColor, fontSize: 24, fontVariationSettings: "'FILL' 1" }}>
                {cfg.icon}
            </span>
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                boxShadow: "0 25px 50px -12px rgba(0,0,0,0.15)", animation: "modalSlideUp .3s ease-out",
                padding: 32, textAlign: "center"
            }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#fef2f2", color: "#f87171", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                    <span className="material-symbols-outlined" style={{ fontSize: 32 }}>report</span>
                </div>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                    <button onClick={onCancel} style={{ flex: 1, padding: "12px 0", borderRadius: 12, border: "1px solid #e2e8f0", background: "white", fontWeight: 700, color: "#64748b", cursor: "pointer" }}>Hủy bỏ</button>
                    <button onClick={onConfirm} disabled={loading} style={{ flex: 1, padding: "12px 0", borderRadius: 12, border: "none", background: "#ef4444", fontWeight: 700, color: "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        {loading ? <div style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "white", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} /> : "Xác nhận xóa"}
                    </button>
                </div>
            </div>
        </div>
...
const modalContentStyle = {
    background: "white", borderRadius: 28, boxShadow: "0 25px 50px -12px rgba(0,0,0,0.15)",
    animation: "modalSlideUp .3s ease-out", overflow: "hidden", display: "flex", flexDirection: "column"
};

// ─── Detail Modal ─────────────────────────────────────────────────────────────
function DetailModal({ open, item, onClose }) {
    if (!open || !item) return null;
    const imgs = parseImages(item.imageUrl);
    ...
    return (
        <div style={modalOverlayStyle} onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div style={{ ...modalContentStyle, width: "100%", maxWidth: 840, maxHeight: "85vh" }}>
                <div style={{ padding: "28px 32px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                        <span style={{ fontSize: 11, fontWeight: 800, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em" }}>Chi tiết sự cố #{item.id}</span>
                        <h3 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: "4px 0 0" }}>Biên bản báo cáo vật tư</h3>
                    </div>
                    <button onClick={onClose} style={{ width: 40, height: 40, borderRadius: 12, border: "1px solid #f1f5f9", background: "white", cursor: "pointer", color: "#64748b" }}>
                        <span className="material-symbols-outlined">close</span>
                    </button>
                </div>
                <div style={{ padding: 32, overflowY: "auto", display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 32 }}>
                    <div>
                        <h4 style={{ fontSize: 12, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 16 }}>Minh chứng hiện trường</h4>
                        {imgs.length > 0 ? (
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                {imgs.map((img, i) => (
                                    <div key={i} style={{ borderRadius: 16, overflow: "hidden", border: "1px solid #e2e8f0", boxShadow: "0 4px 12px rgba(0,0,0,0.03)", cursor: "zoom-in" }} onClick={() => window.open(img.url, '_blank')}>
                                        <img src={img.url} alt="Damage" style={{ width: "100%", height: "100%", objectFit: "cover", aspectRatio: "4/3", display: "block" }} />
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div style={{ padding: "50px 0", textAlign: "center", background: "#f8fafc", borderRadius: 20, border: "2px dashed #e2e8f0" }}>
                                <span className="material-symbols-outlined" style={{ fontSize: 36, color: "#cbd5e1" }}>no_photography</span>
                                <p style={{ fontSize: 13, color: "#94a3b8", marginTop: 8 }}>Không có ảnh chứng cứ</p>
                            </div>
                        )}
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                        <div>
                             ...
                             <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                                ...
                                <div style={{ display: "flex", gap: 12 }}>
                                    <div style={{ flex: 1, background: "#f8fafc", padding: "14px 18px", borderRadius: 16 }}>
                                        <p style={{ fontSize: 11, fontWeight: 700, color: "#64748b", margin: "0 0 4px" }}>SỐ LƯỢNG</p>
                                        <p style={{ fontSize: 18, fontWeight: 800, color: "#0f172a", margin: 0 }}>{item.quantity}</p>
                                    </div>
                                    <div style={{ flex: 1, background: "#fff1f2", padding: "14px 18px", borderRadius: 16 }}>
                                        <p style={{ fontSize: 11, fontWeight: 700, color: "#e11d48", margin: "0 0 4px" }}>ĐỀN BÙ</p>
                                        <p style={{ fontSize: 18, fontWeight: 800, color: "#e11d48", margin: 0 }}>{fmtCurrency(item.penaltyAmount)}</p>
                                    </div>
                                </div>
                                <div style={{ background: st.bg, padding: "14px 18px", borderRadius: 16, border: `1px solid ${st.color}20` }}>
                                    <p style={{ fontSize: 11, fontWeight: 700, color: st.color, margin: "0 0 4px" }}>TRẠNG THÁI</p>
                                    <p style={{ fontSize: 15, fontWeight: 800, color: st.color, margin: 0 }}>{st.label}</p>
                                </div>
                                <div style={{ background: "white", padding: "16px 20px", borderRadius: 16, border: "1px solid #f1f5f9" }}>
                                    <p style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", marginBottom: 8 }}>Báo cáo của nhân viên</p>
...
function EditModal({ open, item, onClose, onSaved, showToast }) {
    ...
    useEffect(() => {
        if (item) {
            setForm({ quantity: item.quantity, penaltyAmount: item.penaltyAmount, description: item.description, status: item.status });
            setExistingImages(parseImages(item.imageUrl));
            setNewImageFiles([]); setNewPreviews([]);
        }
    }, [item]);
        ...
        <div style={modalOverlayStyle} onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div style={{ ...modalContentStyle, width: "100%", maxWidth: 560 }}>
                <div style={{ padding: "24px 32px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <h3 style={{ fontSize: 18, fontWeight: 800, color: "#0f172a", margin: 0 }}>Hiệu chỉnh biên bản #{item.id}</h3>
...
export default function LossAndDamagePage() {
    ...
    const fetchRecords = useCallback(async (silent = false) => {
        ...
        try {
            ...
            const res = await axiosClient.get(`/LossAndDamages?${params.toString()}`);
            const data = Array.isArray(res.data) ? res.data : (res.data?.data || []);
            setRecords(data);
            setLastUpdated(new Date());
        } catch { if (!silent) showToast("Lỗi kết nối máy chủ.", "error"); }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            setDeleteTarget(null); fetchRecords(true);
    ...
    return (
        <>
            ...
            <div style={{ position: "fixed", bottom: 40, right: 40, zIndex: 2000, pointerEvents: "none" }}>{toasts.map(t => <Toast key={t.id} {...t} onDismiss={dismissToast} />)}</div>
            <DetailModal open={!!detailItem} item={detailItem} onClose={() => setDetailItem(null)} />
            <EditModal open={!!editItem} item={editItem} onClose={() => setEditItem(null)} onSaved={() => fetchRecords(true)} showToast={showToast} />
            <ConfirmDialog open={!!deleteTarget} title="Xác nhận xóa" message="Hành động này sẽ xóa vĩnh viễn biên bản và tất cả hình ảnh liên quan." onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} loading={deleteLoading} />

            <div style={{ maxWidth: 1400, margin: "0 auto", padding: "40px 20px" }}>
                <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
                    <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#64748b", fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 18 }}>inventory_2</span> Quản lý kho vật tư
                        </div>
                        <h1 style={{ fontSize: 32, fontWeight: 800, color: "#0f172a", margin: 0, letterSpacing: "-0.02em" }}>Thất thoát &amp; Đền bù</h1>
                    </div>
                </header>
                ...
                {/* Toolbar */}
                <div style={{ background: "white", borderRadius: 24, padding: "20px 24px", border: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#f8fafc", padding: "8px 16px", borderRadius: 12, border: "1px solid #f1f5f9" }}>
                            <span className="material-symbols-outlined" style={{ fontSize: 18, color: "#94a3b8" }}>calendar_today</span>
                            <input type="date" value={filters.fromDate} onChange={e => setFilters(f => ({ ...f, fromDate: e.target.value }))} style={{ border: "none", background: "none", fontSize: 13, fontWeight: 600, outline: "none", color: "#1e293b" }} />
                ...
                {/* Main Table */}
                <div style={{ background: "white", borderRadius: 28, border: "1px solid #f1f5f9", overflow: "hidden", boxShadow: "0 4px 30px rgba(0,0,0,0.02)" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        <thead>
                            <tr style={{ background: "#fbfbfc" }}>
                                {["ID", "Minh chứng", "Phòng", "Vật liệu sự cố", "Số lượng", "Đền bù", "Thời gian báo cáo", "Thao tác"].map(h => (
                                    <th key={h} style={{ padding: "18px 24px", textAlign: "left", fontSize: 11, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                <tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Đang đồng bộ dữ liệu...</td></tr>
                            ) : paged.length === 0 ? (
                                <tr><td colSpan={8} style={{ padding: 60, textAlign: "center", color: "#94a3b8" }}>Phòng ban chưa ghi nhận sự cố nào</td></tr>
                            ) : (
                                paged.map((rec) => {
                                    const imgs = parseImages(rec.imageUrl);
                                    const dt = fmtDateTime(rec.createdAt);
                                    const st = STATUS_CFG[rec.status] || STATUS_CFG.Pending;
                                    return (
                                        <tr key={rec.id} className="table-row">
                                            ...
                                            <td style={{ padding: "20px 24px" }}>
                                                <div style={{ padding: "4px 10px", borderRadius: 8, background: "#eff6ff", color: "#1d4ed8", fontWeight: 800, fontSize: 14, display: "inline-block" }}>{rec.roomNumber}</div>
                                            </td>
                                            <td style={{ padding: "20px 24px", fontSize: 14, fontWeight: 700, color: "#0f172a" }}>{rec.itemName}</td>
                                            <td style={{ padding: "20px 24px", fontSize: 15, fontWeight: 800, color: "#64748b" }}>{rec.quantity}</td>
                                            <td style={{ padding: "20px 24px", fontSize: 15, fontWeight: 800, color: "#ef4444" }}>{fmtCurrency(rec.penaltyAmount)}</td>
                                            <td style={{ padding: "20px 24px" }}>
                                                <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a" }}>{dt.date}</div>
                                                <div style={{ fontSize: 11, color: "#94a3b8" }}>{dt.time}</div>
                                            </td>
                                            <td style={{ padding: "20px 24px" }}>
                                                <div style={{ display: "flex", gap: 8 }}>
                                                    <button className="btn-icon-p" onClick={() => setDetailItem(rec)} title="Xem chi tiết"><span className="material-symbols-outlined">visibility</span></button>
                                                    <button className="btn-icon-p" onClick={() => setEditItem(rec)} title="Chỉnh sửa"><span className="material-symbols-outlined">edit_square</span></button>
                                                    <button className="btn-icon-p" onClick={() => setDeleteTarget(rec)} title="Xóa" style={{ color: "#ef4444" }}><span className="material-symbols-outlined">delete_forever</span></button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
                    ...
                    <footer style={{ padding: "20px 32px", borderTop: "1px solid #f8fafc", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>Hiển thị <strong>{paged.length}</strong> / <strong>{records.length}</strong> biên bản sự cố</p>
                        <div style={{ display: "flex", gap: 6 }}>
                            <button className="btn-icon-p" disabled={page === 1} onClick={() => setPage(p => p - 1)}><span className="material-symbols-outlined">chevron_left</span></button>
                            {[...Array(totalPages)].map((_, i) => (
                                <button key={i} className="btn-icon-p" onClick={() => setPage(i + 1)} style={page === i + 1 ? { background: "#0f172a", color: "white", borderColor: "#0f172a", fontWeight: 800 } : {}}>{i + 1}</button>
                            ))}
                            <button className="btn-icon-p" disabled={page === totalPages} onClick={() => setPage(p => p + 1)}><span className="material-symbols-outlined">chevron_right</span></button>
                        </div>
                    </footer>
                </div>
            </div>
